...
<div class="popupConsult" id="popupConsult" style="overflow: scroll;" >
    <form action="{{ route('enrOrdDetail') }}" method="POST" class="formIns med">
        @csrf
    <select class="textinsMed solide"  name="consultation">
        ...
        ?>
            @if ( $interval->format('%R%D') < $jour )
                <option><span>Consultation du {{ $consultations->created_at }}</span> <input type="hidden" name="id_cons" value="{{ $consultations->id}}"></option>
            @endif
    </select>
...
<div class="pageMedConDossier" style="background: none">
                ...
                @if(session('ord_first') == true)
                  @foreach ($ordennances as $ordennance)
                    <tr style=" width: 100%;height: 40px;  font-weight: 600; font-size: 900">
                        <td><a href="{{ url('voirOrdDetail/' .$ordennance->id) }}" class="cl">Ordonnance du  {{ $ordennance->created_at }}</a></td>
                    </tr>
                  @endforeach
                @endif
            </table>
    ...
    <script>
        ...
        function suppr(bouton){
            if(document.getElementsByTagName("li").length > 13){
